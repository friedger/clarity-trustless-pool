...
(define-read-only (in-prepare-phase)
  (let (
    ...
    {
      blocks-in-cycle: blocks-in-cycle,
      in-prepare-phase: (> blocks-in-cycle start-prepare-phase),
  ...
  (let (
      ...
      (balance (unwrap!
        (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
          get-balance this-contract
        )
        err-not-found
      ))
    )
    (asserts!
      (or (> blocks-in-cycle start-prepare-phase) (> balance intermed-minimum))
      err-too-early
    )
